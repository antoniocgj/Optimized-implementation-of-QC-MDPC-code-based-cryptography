...
int main(int argc, char const *argv[]) {
  ...
  index_t private_key_rel[2][W/2 + HWPA], private_key_transposed[2][W/2];
  ...
  for (size_t ki = 0; ki < 25 + NUMBER_OF_TESTS; ki++) {
    ...
    /* Key Generation */
    start = RDTSC();
    if(1 == generate_keys(public_key, private_key_rel, private_key_transposed)){
    ...
    /* Decryption */
    start = RDTSC();
    decrypt(private_key_rel, private_key_transposed, ciphertext, res_symmetric_key);
    end = RDTSC();
